...
const fastOptimalSequence = (n) => {
	...
	for (let number = 2; number <= n; number++) {
		const numOpsDivide2 = (number % 2 === 0) ? minNumOps[number / 2] : Number.MAX_VALUE;
		const numOpsDivide3 = (number % 3 === 0) ? minNumOps[number / 3] : Number.MAX_VALUE;
...
const readLine = (line) => {
	const sequence = fastOptimalSequence(parseInt(line, 10));
	process.stdout.write(sequence.length - 1 + '\n');
